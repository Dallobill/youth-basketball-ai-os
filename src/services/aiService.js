...
async function buildPlayerSummary(input) {
  const strengths = safeArray(input.strengths, ['Competes consistently', 'Responds well to coaching']);
  const priorities = safeArray(input.priorities, ['Decision-making under pressure', 'Finishing through contact']);

  return {
    headline: input.headline || 'Player development summary',
    strengths,
    developmentPriorities: priorities,
    coachFocus: input.coachFocus || 'Emphasize repetition, pace control, and clearer reads this week.',
    playerMessage:
      input.playerMessage ||
...
async function buildTeamSummary(input) {
  return {
    ...
    strengths: safeArray(input.strengths, ['Good effort level', 'Strong transition energy']),
    recurringIssues: safeArray(input.recurringIssues, ['Late help defense', 'Rushed offensive decisions']),
    nextPracticeFocus: safeArray(input.nextPracticeFocus, ['Shell defense', 'Advantage decision-making']),
    coachingPoints: safeArray(input.coachingPoints, ['Talk early', 'Play off two feet'])
  };
}

async function buildPracticePlan(input) {
  return {
    practiceTheme: input.practiceTheme || 'Play with balance, discipline, and communication',
    priorities: safeArray(input.priorities, ['Help defense', 'Finishing under pressure', 'Decision-making']),
    segmentPlan: [
    ...
    coachingEmphasis: 'Demand communication, controlled pace, and game-speed execution.'
  };
}
